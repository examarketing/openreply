...
export default function CampaignBuilder({ mode, campaignId }: CampaignBuilderProps) {
  ...
  async function handleSubmit(activeValue: boolean) {
    ...
    if (!dmMessage.trim()) return setError("Escreva a DM (a mensagem que a pessoa vai receber).");
      ...
      <div className="space-y-8 min-w-0">
        ...
        <div className="space-y-3">
          ...
          <input
            ...
            placeholder="ex.: PORTU — comenta MAPA"
        ...
        <Section title="E o comentário tiver">
          ...
          >
            uma palavra (ou palavras) específica
          </Radio>
          {matchMode === "specific" && (
          ...
          {publicReplyEnabled && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted">
                Uma é sorteada a cada vez, pra as respostas não ficarem
                idênticas.
              </p>
            </div>
          )}
            ...
            {requireFollow && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="Antes de te mandar o link: me segue aqui pra não perder os próximos conteúdos. Toca no botão assim que estiver seguindo que eu te envio."
                  rows={3}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none resize-none"
                  maxLength={1000}
                />
                ...
                <p className="text-xs text-muted">
                  ...
                  confirma que ela segue. Se não der pra verificar, enviamos
                  mesmo assim.
                </p>
              </div>
            )}
        ...
        <Section title="E depois, recebe">
          <div className="rounded-lg border border-border p-3 space-y-2">
            ...
            <p className="text-xs text-muted">
              {"{link}"} insere o link rastreado; {"{username}"} coloca o nome da pessoa.
            </p>
          </div>
          <div className="mt-3 rounded-lg border border-border p-3">
            <div className="flex items-center justify-between">
            ...
            {followUpEnabled && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="Obrigado por seguir! Qualquer dúvida é só responder aqui 🙌"
                  rows={3}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none resize-none"
                  maxLength={1000}
                />
