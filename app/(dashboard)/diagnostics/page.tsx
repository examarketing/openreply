...
export default function DiagnosticsPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            Saúde, filas, falhas de webhook e alertas do robô de envio.
          </p>
        </div>
        <button
        ...
        <div className="panel rounded p-4 sm:p-5">
          ...
          <p className="mt-2 text-xs text-muted">
            {workerAgeSeconds == null
              ? "Sem sinal do robô"
              : `Último sinal ${workerAgeSeconds}s atrás`}
          </p>
        </div>
        {["waiting", "active", "delayed", "failed"].map((key) => (
          <div key={key} className="panel rounded p-4 sm:p-5">
            <p className="text-xs font-semibold uppercase text-muted">
              Fila: {({ waiting: "aguardando", active: "processando", delayed: "agendadas", failed: "falhas" } as Record<string, string>)[key]}
            </p>
      ...
      <Section title="Alertas recentes do robô">
        {data?.workerAlerts.length ? (
          ...
          <EmptyState label="Nenhum alerta registrado." />
        )}
      </Section>

      <div className="grid gap-6 lg:grid-cols-2">
        <Section title="DMs que falharam ou foram puladas">
          {data?.dmFailures.length ? (
            ...
            <EmptyState label="Nenhuma falha ou DM pulada." />
          )}
        </Section>
