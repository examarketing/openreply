...
export default function CampaignsPage() {
  ...
  const [copiedId, setCopiadoId] = useState<string | null>(null);
  ...
  async function copyReelUrl(auto: Campaign) {
    ...
    try {
      ...
      setCopiadoId(auto.id);
      window.setTimeout(
        () => setCopiadoId((cur) => (cur === auto.id ? null : cur)),
        1500
      );
    } catch (err) {
  ...
  async function deleteAutomation(id: string) {
    if (!confirm("Excluir esta automação? Não dá pra desfazer.")) return;
    try {
    ...
    <div className="space-y-6">
        ...
        <div>
          <p className="text-sm text-muted">
            ...
            {filtered.length !== automations.length
              ...
              : ""}{" "}
            automaç{automations.length !== 1 ? "ões" : "ão"}
          </p>
        </div>
      ...
      {automations.length === 0 && (
        <div className="panel rounded p-8 text-center sm:p-12">
          <h3 className="text-lg font-semibold mb-2">Nenhuma automação ainda</h3>
          <p className="text-sm text-muted mb-6 max-w-sm mx-auto">
            Crie a primeira automação de comentário → DM e transforme um post ou reel em conversas medidas.
          </p>
          <Link
            href="/campaigns/new"
      ...
      {automations.length > 0 && filtered.length === 0 && (
        <div className="panel rounded p-8 text-center text-sm text-muted">
          Nenhuma automação bate com a busca.
        </div>
      )}
            ...
            <div className="flex flex-wrap items-start gap-x-4 gap-y-3">
              ...
              <div className="min-w-[12rem] flex-1">
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-3 text-xs text-zinc-500">
                  ...
                  <span>·</span>
                  <span>{auto.analytics.skipped} puladas</span>
                  <span>·</span>
